  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        // Erro específico: função RPC não existe no banco
        if (
          error.code === 'PGRST202' ||
          ...
          throw new Error(
            'A função "delete_contact_and_messages" não foi criada no banco de dados. ' +
            'Execute o arquivo SQL "delete_contact_and_messages.sql" no Supabase SQL Editor e tente novamente.'
          );
        }
